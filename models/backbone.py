...
def initialize_pretrained_model(model, settings):
    weights_path = os.path.join(os.path.join(os.path.dirname(__file__), '..',
                                        'pretrainedmodels', os.path.basename(settings['url'])))
    state_dict = torch.load(weights_path)
    # filter out last layer
    for layer in ['last_linear.weight', 'last_linear.bias']:
